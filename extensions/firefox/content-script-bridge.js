    ...
    async function initializeScripts() {
        try {
            ...
            if (
                ...
                window.wrappedJSObject.GM_ready
            ) {
      ...
      `;
                ...
                if (document.readyState === 'loading') {
                    document.addEventListener('DOMContentLoaded', () => {
                        setTimeout(
                            () =>
                                injectPageScript(
                                    'scripts/better-moodle-uzl.user.js'
                                ),
                            100
                        );
                    });
            ...
            console.error(
                '[Better-Moodle Extension] Failed to inject scripts:',
                error
            );
        }
    }
            ...
            switch (type) {
                ...
                case 'NOTIFICATION': {
                    ...
                    const browserNotificationOptions = {
                        type: 'basic',
                        iconUrl:
                            image || browser.runtime.getURL('icon-128.png'),
                        title: title,
                        message: text,
                    };
