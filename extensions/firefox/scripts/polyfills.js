/**
 * Firefox Extension Polyfills for Page Context
 *
 ...
 *
 * IMPORTANT: This file is injected into the page and does NOT have access to browser APIs!
 */
...
(function () {
    ...
    function sendMessage(type, data) {
        return new Promise((resolve, reject) => {
            ...
            window.postMessage(
                { source: 'better-moodle-page', type, data, id },
                '*'
            );
    ...
    window.addEventListener('message', event => {
        ...
        if (!event.data || event.data.source !== 'better-moodle-content')
            return;
            ...
            if (valueChangeListeners[key]) {
                valueChangeListeners[key].forEach(listener => {
                    try {
                        ...
                        console.error(
                            'GM_addValueChangeListener callback error:',
                            error
                        );
                    }
                });
        ...
        initPromise = (async () => {
            ...
            while (retries < 50) {
                ...
                try {
                    ...
                    console.log(
                        '[Better-Moodle Extension] Storage cache initialized'
                    );
                    return;
                } catch (error) {
                    retries++;
                    if (retries >= 50) {
                        console.error(
                            '[Better-Moodle Extension] Failed to initialize cache after retries:',
                            error
                        );
                        throw error;
                    }
    ...
    window.GM_addValueChangeListener = function (key, callback) {
        ...
        valueChangeListeners[key].push({ id: listenerId, callback: callback });
    ...
    window.GM = {
        xmlHttpRequest: function (details) {
            ...
            const responseListener = event => {
                ...
                if (
                    !event.data ||
                    event.data.source !== 'better-moodle-content'
                )
                    return;
                if (!event.data.type || !event.data.type.startsWith('XHR_'))
                    return;
    ...
    console.log(
        '[Better-Moodle Extension] GM_* APIs initialized (page context)'
    );
})();
