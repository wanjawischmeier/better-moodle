...
(async function() {
  ...
  function loadScript(url) {
    return new Promise((resolve, reject) => {
      ...
      script.onerror = (error) => {
        console.error(`[Better-Moodle Extension] Failed to load ${url}:`, error);
        reject(error);
      };
  ...
  try {
    ...
    await loadScript(browser.runtime.getURL('scripts/better-moodle-uzl.user.js'));
    
    console.log('[Better-Moodle Extension] All scripts loaded successfully');
  } catch (error) {
    console.error('[Better-Moodle Extension] Failed to load scripts:', error);
  }
})();
