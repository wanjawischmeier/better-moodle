...
(async function() {
  ...
  if (!stackMatch) {
    console.error('[Better-Moodle Extension] Could not determine extension ID from stack trace');
    return;
  }
  ...
  function loadScriptSync(url) {
    ...
    if (xhr.status === 200) {
      ...
      console.error(`[Better-Moodle Extension] Failed to load ${url}:`, xhr.status);
      return false;
    }
  }
  ...
  try {
    loadScriptSync(`chrome-extension://${extensionId}/scripts/darkreader.js`);
  ...
  loadScriptSync(`chrome-extension://${extensionId}/scripts/better-moodle-uzl.user.js`);
